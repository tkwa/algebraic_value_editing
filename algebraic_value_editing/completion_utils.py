...
def gen_using_model(
    ...
    **sampling_kwargs,
    ...
    returns:
        ...
        has the following columns:
                ...
                `is_modified`: Whether the completion was modified by
                    any hook functions.activation_additions: List[ActivationAddition] = [
        *get_x_vector(
            prompt1=prompt1,
            prompt2=prompt2,
            coeff=x,
    ...
    if include_full_cache:
        logits, mcache = model.run_with_cache(completions.clone())
        final_list = []
        for i in range(len(results)):
            resid_keys = ['blocks.0.hook_resid_pre'] + [k for k in mcache.keys() if 'hook_resid_post' in k]
            final_list.append(t.stack([mcache[k][i] for k in resid_keys], dim=0).detach().cpu())
        results["full_cache"] = final_list
